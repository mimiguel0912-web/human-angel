...
public class Main extends JavaPlugin implements CommandExecutor, Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (c) {
            ...
            case "pos1": 
                ...
                p.sendMessage("§b§l[WAND] §fPosição 1 marcada em: " + p.getLocation().getBlockX() + ", " + p.getLocation().getBlockY() + ", " + p.getLocation().getBlockZ());
                break;
            case "pos2": 
                pos2.put(p.getUniqueId(), p.getLocation()); 
                p.sendMessage("§d§l[WAND] §fPosição 2 marcada em: " + p.getLocation().getBlockX() + ", " + p.getLocation().getBlockY() + ", " + p.getLocation().getBlockZ());
                break;
            case "angelwand":
                p.getInventory().addItem(new ItemStack(Material.TRIDENT));
                p.sendMessage("§bTridente de construção entregue!"); break;
            case "set": fill(p, args, false); break;
            case "paredes": fill(p, args, true); break;
            case "lista":
                p.sendMessage("§b§l--- LISTA DE COMANDOS ---");
                p.sendMessage("§f/home, /sethome, /spawn, /pvp, /tpa, /tpaccept, /tpadeny, /lista");
                if (p.isOp()) p.sendMessage("§e/modo, /escala, /sc, /set, /paredes, /pos1, /pos2, /angelwand");
                break;
            case "modo":
                if (args.length > 0) {
    ...
    private void fill(Player p, String[] args, boolean walls) {
        ...
        if (l1 == null || l2 == null || args.length == 0) { p.sendMessage("§cMarque as 2 posições (/pos1 e /pos2) e use /set [bloco]"); return; }
        ...
        p.sendMessage("§aBlocos colocados com sucesso!");
    }
